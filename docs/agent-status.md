...
- DONE (2026-02-25): Issue #9 completed in commit `4f73ead`. Verification: `node --check` passed for `apps/web/app/page.js`, `apps/web/app/shop/page.js`, `apps/web/app/not-found.js`, and `apps/web/scripts/route-check.mjs`; `node apps/web/scripts/route-check.mjs` returned `route-check: ok (2 routes, 4 app source files scanned)`. Handoff: route checker currently validates only literal internal href strings and required static routes (`/`, `/shop`) plus `app/not-found.js`.
